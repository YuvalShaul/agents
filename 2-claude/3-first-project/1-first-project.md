...
already covered by `2-claude/.gitignore`, so it won't get committed, but
keep the directory itself around.
...
1. Create the project directory and start Claude Code in it, from inside
   `2-claude/3-first-project/`:
   ```
   mkdir firstproj
   cd firstproj
   claude
